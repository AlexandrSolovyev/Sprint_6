...
class OrderPage(BasePage):
    url_order = settings.URL_ORDER

    @allure.step(f"Переход на страницу оформления заказа {url_order}")
    def go_to_url_order(self):
        self.driver.get(self.url_order)

    @allure.step('Ввод данных в поля "Имя, Фамилия, Адрес, Номер телефона"')
    def set_text_to_first_form(
            self,
            nam_locator,
            name,
            se_na_locator,
    ...
    @allure.step(
        'Нажатие на кнопку "Заказать", "Да" и получение текста с окна поддверждения'
    )
    def click_on_order_and_get_text_notification(
